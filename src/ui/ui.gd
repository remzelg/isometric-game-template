...
@onready var pause_menus: Array[CanvasItem] = [
	$InGameMenuOverlay, $Settings, $PauseMenu, $Confirmation
]
...
# Fades to black, swaps the current scene, then fades back in. Use this
# instead of calling get_tree().change_scene_to_file() directly whenever the
# transition should be covered rather than an instant hard cut.
func fade_to_scene(path: String, duration: float = 0.3) -> void:
	await scene_transition.fade_out(duration)
	get_tree().change_scene_to_file(path)
	await get_tree().process_frame
	await scene_transition.fade_in(duration)


func get_page(page: String) -> UiPage:
	return _resolve_ui_page(page)
